...
export default function ProfileContent() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      const res = await customerService.updateMyProfile(updateData);
      if (res.success) {
        ...
        setUpdateMessage({ type: "error", text: res.message || "Cập nhật thất bại!" });
      }
    } catch (err: any) {
      console.error("Update profile failed", err);
      setUpdateMessage({ type: "error", text: err.response?.data?.message || "Lỗi hệ thống khi cập nhật hồ sơ" });
    } finally {
                ...
                <tbody className="divide-y divide-gray-50">
                  {orders.map(order => (
                    <tr key={order.id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-8 py-6">
                        <span className={`inline-block px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${order.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-600' :
                      ...
                      return (
                        <tr key={res.id} className="hover:bg-gray-50/50 transition-colors group">
                          ...
                          <td className="px-10 py-7">
                            <span className={`inline-block px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${res.status === 'CONFIRMED' ? 'bg-emerald-50 text-emerald-600' :
